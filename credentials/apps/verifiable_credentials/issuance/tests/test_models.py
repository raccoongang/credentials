...
class IssuanceLineTestCase(SiteMixin, TestCase):
    ...
    def test_issuer_name_property(self, mock_get_issuer):
        self.issuance_line.issuer_name
    ...
    def test_get_status_list_url_without_hash_param(self, mock_get_current_request):
        mock_get_current_request.return_value = APIRequestFactory(SERVER_NAME=self.site.domain).get("/")
        status_list_url = self.issuance_line.get_status_list_url()
        # TODO: find way to test it

    @mock.patch("credentials.apps.verifiable_credentials.issuance.models.get_current_request")
    def test_get_status_list_url_with_hash_param(self, mock_get_current_request):
        mock_get_current_request.return_value = APIRequestFactory(SERVER_NAME=self.site.domain).get("/")
        status_list_url = self.issuance_line.get_status_list_url(hash_str="test_hash")
        self.assertIn("#test_hash", status_list_url)
        # TODO: find way to test it
    ...
    def test_get_next_status_index_not_first(self):
        IssuanceLineFactory.create_batch(3, issuer_id="test-issuer-id")
        next_status_index = IssuanceLine.get_next_status_index("test-issuer-id")
        # FIXME: factory instance id cached
        # self.assertEqual(next_status_index, 3)

    def test_get_indicies_for_status(self):
        IssuanceLineFactory.create_batch(
            2, issuer_id="test-issuer-id", processed=True, user_credential__status=UserCredentialStatus.AWARDED
        )
        IssuanceLineFactory.create_batch(
            3,
        ...
        self.assertEqual(indicies, [6, 7, 8])
...
class IssuanceConfigurationTestCase(SiteMixin, TestCase):
    ...
    def test_create_issuers_update(self):
        IssuanceConfigurationFactory.create(
            issuer_id="test-issuer-did",
            issuer_key="test-issuer-key",
        )
        IssuanceConfiguration.create_issuers()
        issuance_configuration = IssuanceConfiguration.objects.last()
        self.assertIsNotNone(issuance_configuration)
